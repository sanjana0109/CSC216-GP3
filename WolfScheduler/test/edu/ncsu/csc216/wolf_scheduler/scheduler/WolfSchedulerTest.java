...
public class WolfSchedulerTest {
	...
	public void testAddCourse() {
		...
		Activity c1 = new Course("CSC226", "Discrete Mathematics for Computer Scientists", "001", 3, "tmbarnes", "MWF", 935, 1025);
		// Add course that overlaps.
		...
		String[] course1 = ws.getFullScheduledActivities()[1];
		assertEquals("CSC226", course1[0]);
		assertEquals("001", course1[1]);
		assertEquals("Discrete Mathematics for Computer Scientists", course1[2]);
		assertEquals("" + 3, course1[3]);
		assertEquals("tmbarnes", course1[4]);
		assertEquals(c1.getMeetingString(), course1[5]);
		assertEquals("", course1[6]);
	...
	public void testAddEvent() {
		...
		// Add event that overlaps.
		try {
			ws.addEvent("Lunch", EVENT_MEETING_DAYS, 830, 1100, EVENT_WEEKLY_REPEAT, EVENT_DETAILS);
			fail();
		} catch (IllegalArgumentException e) {
	...
	public void testGetScheduledActivities() {
		...
		assertTrue(ws.addCourse("CSC116", "003"));
